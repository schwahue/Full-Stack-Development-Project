...
router.post('/signup', [
    ...
    .isLength({ min: 4 }).withMessage('must be at least 4 characters long').bail(),

    body('c_password')
    .notEmpty().withMessage('Invalid Password Comfirmation').bail()
    .custom((value, { req }) => value === req.body.password).withMessage('Password Confirmation must be the same as password'),

    body('contact_number')
    .notEmpty().withMessage('Invalid Contact Number').bail()
    .isLength({ min: 8, max: 8 }).withMessage('must be at least 8 characters long').bail()
    .matches('(8|9)[0-9]{7}').withMessage('Enter numbers only').bail()
    .trim()
    .custom((value, {req}) => {
    ...
    .trim()
    .custom((value, {req}) => {
        return User.findOne({ where: { shop_name: req.body.shop_name } }).then(user => {
            if (user) {
